...
const elements = {
  ...
  systemResetButton: document.querySelector("#system-reset-button"),
  systemResetFeedback: document.querySelector("#system-reset-feedback"),
...
function restoreResettableUiState() {
  ...
  state.filters.employeeFocus = "all";
  state.filters.activitySearch = "";
  state.filters.activityType = "all";
  state.filters.activitySeverity = "all";
  ...
  elements.activitySeverityFilter.value = "all";
  elements.studioRepeatInput.value = "1";
  elements.manualRepeatInput.value = "1";
  elements.manualContextInput.value = "";
  elements.studioLaunchFeedback.textContent =
    "Launch a scenario to drive live activity through the same scoring and alert pipeline as real events.";
  elements.manualSendFeedback.textContent =
    "This sends a real ingestion payload through the live endpoint using your signed-in admin session.";
}
...
async function resetSystemState() {
  elements.systemResetButton.disabled = true;
  elements.systemResetFeedback.textContent = "Resetting SentraGuard back to a clean baseline...";

  try {
    const response = await api.resetState();
    restoreResettableUiState();
    await Promise.all([loadSystemGuide(), loadControlScenarios(), refreshDashboard()]);
    updateModeUi(response.mode);
    updateSimulationTempoUi(response.tempo);
    renderInspector(elements.employeeInspector, null);
    renderControlResult(elements.studioQuickResult, null);
    elements.systemResetFeedback.textContent = `${response.total_employees} employees reseeded. Alerts, activity, and risk scores are back to zero.`;
  } catch (error) {
    elements.systemResetFeedback.textContent = error.message || "Reset failed";
    handleUnauthorized(error);
  } finally {
    elements.systemResetButton.disabled = false;
  }
}
...
function connectSocket() {
  ...
  state.socket.addEventListener("message", (event) => {
    ...
    if (
      ...
      if (message.type === "system.reset") {
        const nextTempo = message.payload?.tempo;
        if (nextTempo) {
          updateSimulationTempoUi(nextTempo);
        }
        restoreResettableUiState();
        showToast("System reset", "SentraGuard returned to a clean demo baseline.");
      }
    }
...
elements.systemResetButton.addEventListener("click", () => {
  resetSystemState().catch(handleUnauthorized);
});
